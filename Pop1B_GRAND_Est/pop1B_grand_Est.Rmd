...
=======
# install.packages("OpenMPController")
>>>>>>> 6276cd754f4f979423f59ef0bd0a6d0728b5cbec
...
d <- fread(pathMcBookAir, skip = 10)
=======
source("../Carto_utilitaires.R")
load("../GHT/limites_des_ght.Rda") # ght3 = spatialPolygons
...
dim(com_rge) # com_rge compte 210 colones
...
calcul en fonction des GHT: 
---------------------------
on ne garde que les colonnes avec des résultats.
```{r}
#plot(ght5)
#text(coordinates(ght5), names(ght5))
...
plot(ght3)
a <- com_rge[, 205:ncol(com_rge)]
names(a)

# Tapply par GHT, moyenne
x <- tapply(as.numeric(a$propsupouegal75), a$GHT, mean, na.rm=T)
round(x*100, 2)
```
...
```{r}
text(coordinates(ght3), as.character(round(x[names(ght3)] * 100, 2)))
coul = cut(x, breaks = 5, labels = brewer.pal(n = 5, name = "Blues"))

plot(ght3, col = as.character(coul[as.integer(names(ght3))]), main = "% de 75 ans et plus", axes = TRUE)
text(coordinates(ght3), as.character(round(x[names(ght3)] * 100, 2)))
...
plot(ght3, col = as.character(coul[as.integer(names(ght3))]), main = "% de 18 ans et moins", axes = TRUE)
text(coordinates(ght3), as.character(round(x[names(ght3)] * 100, 2)))
```
...
--------
```{r}
x <- tapply(as.numeric(com_rge$propAdulte), com_rge$GHT, mean, na.rm=T)
round(x*100, 2)
x

coul = cut(x, breaks = 5, labels = brewer.pal(n = 5, name = "Greens"))
plot(ght3, col = as.character(coul[as.integer(names(ght3))]), main = "% de 18 à 75 ans", axes = TRUE)
text(coordinates(ght3), as.character(round(x[names(ght3)] * 100, 2)))
...
coul = cut(prop, breaks = 5, labels = brewer.pal(n = 5, name = "Greens"))
plot(ght5, col = as.character(coul[as.integer(names(ght5))]), main = "% de la population totale du grand est", axes = TRUE)
